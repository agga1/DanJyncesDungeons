...
class World:
    def __init__(self, world_path, db):
        ...
        # start room
        self._curr_room = load_curr_room(db)    # position of the room in world map TODO change to curr_room_pos and make self.curr_room as object
        if self._curr_room is None:  # when new game opens, curr_room in db is not initialized!
            self._curr_room = self._start_room
        ...
        for room in rooms:
            if room != "world.txt":
                ...
                for enemy in range(0, enemies_number):
                    ...
                    if len(self._active_enemies) <= enemy_id: # the first time game opens, active_enemies is empty. default is 1 - enemy active
    ...
    def check_room(self, main_character):
        ...
        elif main_character_pos[1] > self._rooms[self._curr_room[0]][self._curr_room[1]].size[1]*50 - sprite_size[1]:
            self.change_room_and_save("bottom", main_character)
        elif main_character_pos[0] < 0:
            self.change_room_and_save("left", main_character)
        elif main_character_pos[0] > self._rooms[self._curr_room[0]][self._curr_room[1]].size[0]*50 - sprite_size[0]:
    ...
    def change_room_and_save(self, direction, main_character): # TODO: maybe save only in checkpoints? if killed -> reload last checkpoint
        ...
        save_curr_room(self._curr_room, self._db)

        if direction == "top":
            if self._curr_room[1] > 0 and self._rooms[self._curr_room[0]][self._curr_room[1] - 1]:
                self._curr_room[1] -= 1
        elif direction == "bottom":
            if self._curr_room[1] < self._size[1] - 1 and self._rooms[self._curr_room[0]][self._curr_room[1] + 1]:
                self._curr_room[1] += 1
        elif direction == "left":
            if self._curr_room[0] > 0 and self._rooms[self._curr_room[0] - 1][self._curr_room[1]]:
                self._curr_room[0] -= 1
        elif direction == "right":
            if self._curr_room[0] < self._size[0] - 1 and self._rooms[self._curr_room[0] + 1][self._curr_room[1]]:
                self._curr_room[0] += 1

        # setting position of the character
        main_character.set_position(direction, self._rooms[self._curr_room[0]][self._curr_room[1]].size)

    @property
    def curr_room(self):
        """ current room (object) """
        return self._rooms[self._curr_room[0]][self._curr_room[1]]
